...
class AirPlayReceiverProvider(PluginProvider):
    ...
    def _update_cover_art(self, metadata: dict[str, Any]) -> None:
        ...
        if "cover_art_timestamp" in metadata and self._metadata_reader and self._metadata_reader.cover_art_bytes:
            # Use a content hash in the path so each unique image gets its own thumbnail
            # cache entry, preventing the first track's artwork from being permanently cached
            # and served for all subsequent tracks (the thumbnail cache is keyed on provider+path).
            img_hash = hashlib.md5(self._metadata_reader.cover_art_bytes).hexdigest()[:8]  # noqa: S324
