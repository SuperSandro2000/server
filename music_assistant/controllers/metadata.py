...
class MetaDataController(CoreController):
    ...
    def _select_description(
        ...
        """
        Pick the best artist bio from this refresh's candidates.

        Dedicated metadata providers are preferred over music providers because they report
        a known language, letting us honour the user's language choice with confidence; a
        music-provider bio (no reliable language signal) is used only as a fallback. A stored
        preferred-language bio is never downgraded to a less-preferred one.
